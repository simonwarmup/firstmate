...
#
# Both halves of the identity are compared case-insensitively, because every
# forge addressed here (GitHub, GitLab, and Bitbucket Cloud) resolves a
# repository path case-insensitively while preserving in the URL whichever
# case it was written in. One repository therefore legitimately arrives here
# spelled two ways - the origin as it was cloned, and the pull request URL as
# the forge or the captain wrote it - and comparing byte-for-byte refused that
# valid destination. This cannot widen the identity to a DIFFERENT repository:
# none of these forges lets two projects on one host differ only by case.
git_origin_matches_repo() {
  ...
  path_part=$(printf '%s' "$path_part" | tr '[:upper:]' '[:lower:]')
