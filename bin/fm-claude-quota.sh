...
#
# --model <model>     the model probed (default: claude-3-5-haiku-20241022).
#                      Any current Claude model works; a small/cheap one keeps
#                      the probe near its minimum cost.
...
# never otherwise printed, logged, or included in any error message.
#
...
# A key whose header this account/plan did not return prints "unknown" -
# never a fabricated zero and never a hard failure, because a different
# account, plan, or Anthropic rollout can return a different subset of the
# documented anthropic-ratelimit-unified-* headers. An unrecognised header
# present in the response is simply ignored.
#
...
# non-2xx response, or a 2xx response that carries none of the
# anthropic-ratelimit-unified-* headers at all (as opposed to only some of
# them, which is the normal, expected, per-plan case handled above). None of
# these print any "key=value" line - a caller must never mistake a failed
# probe for a real all-unknown reading.
set -u
export LC_ALL=C
set +o xtrace
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --timeout=*)
      ...
      ;;
    --help|-h)
      fm_claude_quota_usage "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fm-claude-quota.sh" 2>/dev/null || true
      exit 0
      ;;
    *)
...
[ -n "$MODEL" ] || { printf 'fm-claude-quota.sh: --model must not be empty.\n' >&2; exit 2; }
...
trap 'fm_claude_quota_cleanup; rm -f "$RESPONSE_BODY_FILE"' EXIT
trap 'fm_claude_quota_cleanup; rm -f "$RESPONSE_BODY_FILE"; exit 129' HUP
trap 'fm_claude_quota_cleanup; rm -f "$RESPONSE_BODY_FILE"; exit 130' INT
trap 'fm_claude_quota_cleanup; rm -f "$RESPONSE_BODY_FILE"; exit 143' TERM
...
case "$HTTP_CODE" in
  2??) ;;
  *)
    printf 'fm-claude-quota.sh: the probe call to /v1/messages returned HTTP %s, not a quota reading.\n' "$HTTP_CODE" >&2
...
# HEADERS_FILE, trimmed and with a trailing CR stripped, or empty when the
# header is absent. Grep -i so a lowercased HTTP/2 response still matches.
...
if [ "$HEADER_HITS" -eq 0 ]; then
  printf 'fm-claude-quota.sh: the probe succeeded (HTTP %s) but returned no anthropic-ratelimit-unified-* headers at all; treating this as a probe failure rather than an all-unknown reading.\n' "$HTTP_CODE" >&2
  exit 1
fi

fm_claude_quota_or_unknown() {
  local value
  value=$(fm_claude_quota_field "$1")
  [ -n "$value" ] && printf '%s\n' "$value" || printf 'unknown\n'
}

printf 'unified_status=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-status)"
printf 'five_hour_status=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-5h-status)"
printf 'five_hour_reset=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-5h-reset)"
printf 'five_hour_utilization=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-5h-utilization)"
printf 'seven_day_status=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-7d-status)"
printf 'seven_day_reset=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-7d-reset)"
printf 'seven_day_utilization=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-7d-utilization)"
printf 'seven_day_surpassed_threshold=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-7d-surpassed-threshold)"
printf 'overage_status=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-overage-status)"
printf 'overage_reset=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-overage-reset)"
printf 'overage_utilization=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-overage-utilization)"
printf 'overage_surpassed_threshold=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-overage-surpassed-threshold)"
printf 'overage_disabled_reason=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-overage-disabled-reason)"
printf 'representative_claim=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-representative-claim)"
printf 'upgrade_paths=%s\n' "$(fm_claude_quota_or_unknown anthropic-ratelimit-unified-upgrade-paths)"
